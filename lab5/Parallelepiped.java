public class Parallelepiped {
    ...
    public Coordinate[][] PerspectiveforXY(double k) {
        var lines = new Coordinate[sides.length * 4][2];
        ...
        for (int i = 0; i < sides.length; i++) {
            ...
            for (int j = 0; j < 4; j++) {
                ...
                var vertex2 = edges[j].getP2();
                lines[4 * i + j][0] = new Coordinate(p1.getX(), p1.getY())
                        .multiply(1 / (k * p1.getZ() + 1));
                lines[4 * i + j][1] = new Coordinate(vertex2.getX(), vertex2.getY())
                        .multiply(1 / (k * vertex2.getZ() + 1));
            }
        }
        return lines;
    }

    public Coordinate[][] PerspectiveforXY(double k, Coordinate3D viewDirection) {
        var lines = new Coordinate[sides.length * 4][2];
        ...
        for (int i = 0; i < sides.length; i++) {
            Edge3D[] edges = sides[i].getEdges();
            Coordinate3D normalVector = sides[i].modify(T).getNormal();
