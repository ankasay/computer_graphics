...
// кривые на общем рисунке
        ...
        var canvasForBezier = new DrawingBoardNew(800, 800);
        ...
        var DrawingBoard1 = new DrawingBoardNew(800, 800);
        var DrawingBoard2 = new DrawingBoardNew(800, 800);
        var DrawingBoard3 = new DrawingBoardNew(800, 800);
        ...
        // отметим точки
        for (int i = 0; i < 4; i++) {
            DrawingBoard1.drawPoint(curve1[i], DrawingBoard.Color.RED, 2);
            DrawingBoard2.drawPoint(curve2[i], DrawingBoard.Color.RED, 2);
            DrawingBoard3.drawPoint(curve3[i], DrawingBoard.Color.RED,2);
        }
        ...
        var canvasCutting = new DrawingBoardNew(800, 800);
        ...
        var canvasCutting2 = new DrawingBoardNew(800, 800);
...
class DrawingBoardNew extends DrawingBoard{
    ...
    public void drawPoint(Coordinate p, Color color, int radius) {
        drawPoint(p, color.getBgr(), radius);
    }

    public void drawPoint(Coordinate p, byte[] bgr, int radius) {
        for (int dx = -radius; dx <= radius; dx++) {
            for (int dy = -radius; dy <= radius; dy++) {
                if (dx * dx + dy * dy <= radius * radius) {
                    drawPoint((int) Math.round(p.getX()) + dx, (int) Math.round(p.getY()) + dy, bgr);
                }
            }
        }
    }
